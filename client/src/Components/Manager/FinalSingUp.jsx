...
const FinalManagerSignUp = ({ onClose }) => {
  ...
  const onFinish = () => {
    //כאן יש קריאה לשרת לפונקצייה יצרת מנהל
    createManagerFinal()
...
const user =  useSelector((state) => state.user);
  async function createManagerFinal() {
    try {
      const res = await axios.post(
        ...
        { Status: user.Status, RefId: user._id },
 ...
 dispatch(setUser({ Status: userStatus.ACTIVE }));
      ...
      // Optionally, you might want to stay on the modal or log more detailed error information
    }
  }
